...
export async function triggerWorkflow({github, token}: RequestOptions): Promise<void> {
...
export async function listRecentRuns({github, token}: RequestOptions, limit = 10): Promise<DeployRun[]> {
  const {owner, repo, workflowId, branch} = github
  const data = await ghFetch(`/repos/${owner}/${repo}/actions/workflows/${workflowId}/runs?branch=${branch}&per_page=${limit}&event=workflow_dispatch`, token)
