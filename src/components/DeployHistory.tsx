...
export function DeployHistory({ github, githubToken, documentTypes, titleField }: DeployHistoryProps) {
  ...
  const { data, isLoading, error } = useSWR<DeployRun[]>(
    ["deploy-history", github.owner, github.repo],
    () => listRecentRuns(opts),
    {
  ...
  const apiHasActiveRun = data?.some(
    (r) => r.status === "queued" || r.status === "in_progress",
  );
  ...
  const { data: changedDocs } = useSWR<ChangedDocument[]>(
    trackChanges && lastDeployedAt
      ? ["undeployed-changes", lastDeployedAt, documentTypes]
      : null,
    () =>
      sanityClient.fetch(
    ...
    <Card padding={4} radius={2} shadow={1}>
      <Stack space={4}>
        <Flex justify="space-between" align="center">
          ...
          <Button
            text={isTriggering ? "Triggering..." : "New Deployment"}
            tone="primary"
            onClick={trigger}
          ...
          <Card padding={3} radius={2} border tone="transparent">
            <Stack space={3}>
              <Flex gap={2} align="center">
                <Badge tone="positive" fontSize={1}>Live</Badge>
                <Text size={1}>
                  {formatDate(new Date(lastCompletedRun.createdAt))}
                  {" — "}
                  {lastCompletedRun.commitMessage ?? lastCompletedRun.headSha.slice(0, 7)}
                  {" "}
                  <span style={{ opacity: 0.5 }}>({lastCompletedRun.headSha.slice(0, 7)})</span>
                </Text>
              </Flex>
                ...
                <Stack space={2}>
                  ...
                  {changedDocs.map((doc) => (
                    <Flex key={doc._id} gap={2} align="center">
                      ...
                      <Text size={0} muted>{formatRelative(new Date(doc._updatedAt))}</Text>
                    </Flex>
                  ))}
                </Stack>
              ) : trackChanges && changedDocs ? (
                <Text size={1} muted>All content is up to date.</Text>
              ) : null}
            </Stack>
          </Card>
        )}
                ...
                <Card key={run.id} padding={3} radius={2} border>
                  <Flex gap={4} align="center">
                    ...
                    <Stack space={1} style={{ width: 80, flexShrink: 0 }}>
                      ...
                      {isFinished ? (
                        <Text size={1}>{formatDuration(new Date(run.updatedAt).getTime() - new Date(run.createdAt).getTime())}</Text>
                      ) : (
                        <LiveDuration since={run.createdAt} />
                      )}
                    </Stack>
                    ...
                    <Flex gap={2} align="center">
                      ...
                      <Badge
                        tone={statusTones[run.status] ?? "default"}
                        fontSize={1}
                      >
                        {run.status}
                      </Badge>
